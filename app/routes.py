...
def dashboard_plants_all():
    ...
    if request.method == 'GET':
        spec_table = spec_calc(formdata_list)
        return render_template('plants_dashboard_all.html',spec_table = spec_table,check_filter_all=True, filterform=filterform,round=round, formData=sum_formdata, user=current_user, form=formdata_list)
    else:
        if filterform.validate_on_submit():
            formdata_list = Form.query.filter(Form.kato_6.startswith(filterform.kato_4.data)).all()
...
def dashboard_animals_all():
    ...
    if request.method == 'GET':
        ...
        return render_template('animal_dashboard_all.html',spec_table = spec_calc(formdata_list),check_filter_all=True, filterform=filterform,round=round, formData=sum_formdata, user=current_user, form=formdata_list)
    else:
        if filterform.validate_on_submit():
            formdata_list = Form.query.filter(Form.kato_6.startswith(filterform.kato_4.data)).all()
            formdata_list_go = Form_G_O.query.filter(Form_G_O.kato_6.startswith(filterform.kato_4.data)).all()
